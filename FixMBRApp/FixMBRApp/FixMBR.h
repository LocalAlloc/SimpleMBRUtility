...
void fixwindowsinstallation() {
    if (MessageBoxA(NULL, "Do You Have A Boot Loader Backup?", "Question", MB_YESNO | MB_ICONQUESTION) == IDYES) {
        ...
        // Add Windows installation to the boot list
        char command[100];
        sprintf_s(command, "bootrec /fixmbr");
        Sleep(2000);
        sprintf_s(command, "bootrec /fixboot");
        Sleep(2000);
        sprintf_s(command, "bootrec /rebuildbcd");
...
//    if (FAILED(hr))
//    {
//        logtofile("Failed to initialize COM library.\n");
//        return;
//    }
//
//    ITaskService* pService = NULL;
...
//    if (FAILED(hr))
//    {
//        logtofile("Failed to create Task Scheduler instance.\n");
//        CoUninitialize();
//        return;
//    }
//
...
//    if (FAILED(hr))
//    {
//        logtofile("Failed to connect to Task Scheduler service.\n");
//        pService->Release();
//        CoUninitialize();
//        return;
//    }
...
//    if (FAILED(hr))
//    {
//        logtofile("Failed to get the root folder of Task Scheduler.\n");
//        pService->Release();
//        CoUninitialize();
//        return;
//    }
//
//    IRegisteredTask* pRegisteredTask = NULL;
//    hr = pRootFolder->GetTask(_bstr_t(L"MBR Utility"), &pRegisteredTask);
//    if (FAILED(hr))
//    {
//        logtofile("Failed to get the registered task.\n");
//        pRootFolder->Release();
//        pService->Release();
//        CoUninitialize();
//        return;
...
//    if (FAILED(hr))
//    {
//        logtofile("Failed to run the task.\n");
...
//    }
//
//    logtofile("Task 'MBR Utility' is running.\n");
//
...
DWORD WINAPI UninstallThread(LPVOID lpParam) {
    ...
    if (DeleteFile(filePath)) {
        ...
        if (DeleteTask(L"MBR Utility")) {
            MessageBoxW(NULL, L"Uninstallation completed successfully.", L"Uninstall", MB_OK | MB_ICONINFORMATION);
            return 0;
        }
        else {
            //std::wcerr << "Failed to delete the task." << std::endl;
            ExitProcess(1);
        }
    }
    else {
        //std::wcerr << "Failed to delete the file." << std::endl;
        ExitProcess(1);
    }
}
DWORD WINAPI Uninstall(LPVOID lpParam) {
    MSG msg;
    BOOL bRet;
    ...
    while ((bRet = GetMessage(&msg, NULL, 0, 0)) != 0) {
        if (bRet == -1) {
            ...
            if (msg.message == WM_APP && msg.wParam == 1) {
                std::wstring command = reinterpret_cast<wchar_t*>(msg.lParam);
                // Process the command
                if (command == L"/uninstall") {
                    HANDLE hThread = CreateThread(NULL, 0, UninstallThread, NULL, 0, NULL);
                    if (hThread == NULL) {
                        std::cerr << "Failed to create the uninstallation thread." << std::endl;
                        LocalFree(argv);
                        ExitProcess(1);
                    }
                    ...
                    //std::wcerr << "Invalid command." << std::endl;
                    ExitProcess(1);
                }
            }
        }
